...
namespace Family_Planning
{
    public partial class Task : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            try
            {
                if (CheckBox1.Checked == false)
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select id,dob,replace(dss_id,':','') as dssid ,fp_code,husband_name,mw_name,start_date,visit_no from task where status='3'	and (str_to_date(start_date, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y'))  and replace(dss_id,':','') like '%" + txtdssid.Text + "%' order by STR_TO_DATE(start_date,'%d-%m-%Y')", con);
                ...
                else
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select id,dob,replace(dss_id,':','') as dssid ,fp_code,husband_name,mw_name,start_date,visit_no from task where status='3'	 and replace(dss_id,':','') like '%" + txtdssid.Text + "%' order by STR_TO_DATE(start_date,'%d-%m-%Y')", con);
                    MySqlDataAdapter sda = new MySqlDataAdapter();
                    {
                        cmd.Connection = con;
                        sda.SelectCommand = cmd;
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=Pending Task (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
